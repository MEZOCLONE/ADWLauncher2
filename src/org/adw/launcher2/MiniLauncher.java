...
public class MiniLauncher extends ViewGroup implements View.OnLongClickListener, DropTarget,
	...
	public void onDrop(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView, Object dragInfo) {
        ...
        if(accept){
        	...
        	model.deleteItemFromDatabase(mLauncher, info);
        }
    }
    ...
    public void addItemInDockBar(ItemInfo info){
        ...
        switch (info.itemType) {
        case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
            if (info.container == NO_ID || info.container == Favorites.CONTAINER_DRAWER) {
      ...
      //TODO:  view.setBackgroundDrawable(IconHighlights.getDrawable(mLauncher,IconHighlights.TYPE_DOCKBAR));
        addView(view);
        invalidate();
    }

	public boolean onLongClick(View v) {
        if (!v.isInTouchMode() || mLauncher.isWorkspaceLocked()) {
            return false;
        }
	...
	public void reloadIcons(){
		...
		for(int i=0;i<count;i++){
	        ...
	        if(itemInfo instanceof IconItemInfo){
	            final IconItemInfo info=(IconItemInfo)itemInfo;
                final Bitmap icon = info.getIcon(mLauncher.getIconCache());
                if (icon != null) {
                    ((ImageView)cell).setImageDrawable(Utilities.drawReflection(icon, mLauncher));
                }
	        }
        }
	}
	...
	public void updateCounters(String packageName, int counter, int color){
	    ...
	    for(int i=0;i<count;i++){
            ...
            if (tag instanceof ShortcutInfo) {
	          ...
	          //TODO      if(view instanceof CounterImageView)
	          //TODO           ((CounterImageView) view).setCounter(counter, color);
	                //else if
	                view.invalidate();
	          //TODO      mLauncher.getModel().updateCounterDesktopItem(info, counter, color);
	            }
	        }
